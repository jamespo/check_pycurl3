    ...
    def parse_runfile(self):
        ...
        for global_opt in runyaml:
            if global_opt == 'cookiejar':
                ...
                if self.debug:
                    print("DEBUG: Setting global_options.flags to %s" %
                          runyaml['flags'])
    ...
    def create_curl_obj(self):
        ...
        for flag in self.options.flags:
            ...
            if flag == 'resolve':
                ...
                c.setopt(getattr(c, flag.upper()),
                         self.options.flags[flag].split(','))
            elif flag == 'ipresolve':
                # arg is pycurl const
                c.setopt(getattr(c, flag.upper()), getattr(c, self.options.flags[flag].upper()))
            else:
        ...
        if getattr(self.options, 'postdata', None) is not None:
            ...
            for item in self.options.postdata:
                ...
                if check_postdata is not None:
                    ...
                    postdata = self.prev_matches[url_match_stage].group(
                        url_match_num)
        ...
        try:
            ...
            if self.successtest == 'code':
                ...
                search_res = re.search(
                    self.successcheck, self.content, re.MULTILINE)
...
def get_cli_options():
    ...
    parser.add_option("--insecure", dest="insecure", action="store_true",
                      default=False)
    parser.add_option("--useragent", dest="user_agent",
                      default="check_pycurl3")
